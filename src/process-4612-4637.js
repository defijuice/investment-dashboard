...
// 4612: 모태펀드(문화계정) 2025년 9월 수시 접수현황 - 4개 조합, 6개 운용사
const applications = [
  { company: "디에스투자파트너스", category: "문체부 - 신기술", isJoint: false },
  { company: "에스엠컬처파트너스", category: "문체부 - 신기술", isJoint: true },
  { company: "코로프라넥스트코리아", category: "문체부 - 신기술", isJoint: true },
  { company: "웰컴벤처스", category: "문체부 - 신기술", isJoint: true },
  { company: "로간벤처스", category: "문체부 - 신기술", isJoint: true },
  { company: "일신창업투자", category: "문체부 - 신기술", isJoint: false },
];

// 4637: 선정결과 - 1개 조합
const selectedOperators = [
  { company: "디에스투자파트너스", category: "문체부 - 신기술" },
];
...
async function main() {
  ...
  // 1. 파일 ID 조회
  const file4612 = await sheets.findRow("파일", "파일번호", "4612");
  const file4637 = await sheets.findRow("파일", "파일번호", "4637");
  const fileId4612 = file4612 ? file4612["ID"] : null;
  const fileId4637 = file4637 ? file4637["ID"] : null;
  console.log("파일 ID - 4612:", fileId4612, ", 4637:", fileId4637);

  // 2. 출자사업 생성/조회
  const projectName = "모태펀드(문화계정) 2025년 9월 수시 출자사업";
  ...
  // 5. 파일 4612 처리상태 업데이트
  if (file4612) {
    const rowIdx = file4612._rowIndex;
    await sheets.setValues(`파일!F${rowIdx}:I${rowIdx}`, [[
      "완료",
      new Date().toISOString(),
      "",
      `신청조합 4개, 총 신청현황 ${appsToCreate.length}건`
    ]]);
    console.log("파일 4612 처리 완료");
  }
  ...
  // 7. 파일 4637 처리상태 업데이트
  if (file4637) {
    const rowIdx = file4637._rowIndex;
    await sheets.setValues(`파일!F${rowIdx}:I${rowIdx}`, [[
      "완료",
      new Date().toISOString(),
      "",
    ...
    console.log("파일 4637 처리 완료");
  }

  // 8. 출자사업 파일 연결 및 현황 업데이트
  await sheets.updateProjectFileId(project.id, "선정결과", fileId4637);
