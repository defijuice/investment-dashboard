...
import { ReviewSession, prepareReviewData } from './review-workflow.js';
import { findSimilarOperators, interpretScore } from './operator-matcher.js';
...
async function processPair(applicationFileNo, selectionFileNo) {
  ...
  for (const applicant of finalApplicants) {
    ...
    // 중복은 검토 단계에서 이미 필터링됨 (willSkip)
    // 추가 확인 (수정된 경우)
    const existingKey = `${operator.id}|${applicant.category}`;
    if (existingApplications.has(existingKey)) {
      const existing = existingApplications.get(existingKey);
      console.log(`  [건너뜀] ${applicant.name} (${operator.id}) - 이미 ${existing.status}으로 등록됨 (${applicant.category})`);
    ...
    // 새 약어 발견 시 저장 대기열에 추가
    if (isSelected && matchedAlias && !aliasCache.has(matchedAlias)) {
    ...
    if (isSelected && selectionData) {
      currency = selectionData.currency === 'KRW' ? '억원' : selectionData.currency + ' M';
    }
  ...
  for (const s of selected) {
    ...
    if (decision?.useExisting) {
      ...
      if (operatorId) {
        const aliasData = aliasCache.get(s.name);
        operatorName = aliasData.fullName;
        console.log(`    → 약어 매핑: ${s.name} → ${operatorName} (${operatorId})`);
      } else {
        // 이름으로 찾기
        const existing = await sheets.findOperatorByName(s.name);
        if (existing) {
          operatorId = existing['ID'];
          console.log(`    → 기존 운용사: ${operatorId}`);
        } else {
          // 새 운용사 생성
          const operator = await sheets.getOrCreateOperator(s.name, { region: s.region });
          operatorId = operator.id;
          stats.operatorsCreated++;
          console.log(`    → 새 운용사 생성: ${operatorId}`);
        }
      }
    }

    // 이미 등록된 건인지 확인 (운용사ID + 출자분야 조합)
    const existingKey = `${operatorId}|${s.category}`;
    if (existingApplications.has(existingKey)) {
      const existing = existingApplications.get(existingKey);
      console.log(`    → 이미 ${existing.status}으로 등록됨, 건너뜀 (${s.category})`);
      stats.skippedExisting++;
      continue;
    }

    // 신청현황 레코드 생성 (선정)
    const currency = s.currency === 'KRW' ? '억원' : s.currency + ' M';
    ...
    createdAppIds.push(appId);
    console.log(`  [선정-추가] ${operatorName} (${operatorId}) -> ${appId}`);
    stats.newSelected++;
  }
