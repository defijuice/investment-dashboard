...
// 4547: 글로벌 리그 펀드(문체부 소관) 2025년 접수현황 - 8개 조합, 9개 운용사
const applications = [
  { company: "C Capital Investment Management Limited", category: "문체부 - 글로벌리그", isJoint: false },
  { company: "FAO Partners Pte.Ltd.", category: "문체부 - 글로벌리그", isJoint: false },
  { company: "1686 Partners GP S.a.r.l.", category: "문체부 - 글로벌리그", isJoint: false },
  { company: "Shorooq Partners Ltd", category: "문체부 - 글로벌리그", isJoint: false },
  { company: "BPI Management GmbH", category: "문체부 - 글로벌리그", isJoint: false },
  { company: "Atwater Capital LLC", category: "문체부 - 글로벌리그", isJoint: false },
  { company: "Plug and Play Tech Center", category: "문체부 - 글로벌리그", isJoint: false },
  { company: "Crossbridge Capital Asia Pte.Ltd.", category: "문체부 - 글로벌리그", isJoint: true },
  { company: "스프링캠프", category: "문체부 - 글로벌리그", isJoint: true },
];

// 4619: 선정결과 - 2개 조합
const selectedOperators = [
  { company: "1686 Partners GP S.a.r.l.", category: "문체부 - 글로벌리그" },
  { company: "Shorooq Partners Ltd", category: "문체부 - 글로벌리그" },
];
...
async function main() {
  ...
  // 1. 파일 ID 조회
  const file4547 = await sheets.findRow("파일", "파일번호", "4547");
  const file4619 = await sheets.findRow("파일", "파일번호", "4619");
  const fileId4547 = file4547 ? file4547["ID"] : null;
  const fileId4619 = file4619 ? file4619["ID"] : null;
  console.log("파일 ID - 4547:", fileId4547, ", 4619:", fileId4619);

  // 2. 출자사업 생성/조회
  const projectName = "글로벌 리그 펀드(문체부) 2025년 출자사업";
  ...
  // 5. 파일 4547 처리상태 업데이트
  if (file4547) {
    const rowIdx = file4547._rowIndex;
  ...
  // 7. 파일 4619 처리상태 업데이트
  if (file4619) {
    const rowIdx = file4619._rowIndex;
    await sheets.setValues(`파일!F${rowIdx}:I${rowIdx}`, [[
      "완료",
      new Date().toISOString(),
      "",
    ...
    console.log("파일 4619 처리 완료");
  }

  // 8. 출자사업 파일 연결 및 현황 업데이트
  await sheets.updateProjectFileId(project.id, "선정결과", fileId4619);
